...
func (a automate) Equals(b automate) bool {
	if a.n != b.n || a.m != b.m || a.q0 != b.q0 {
		return false
	}
	if len(a.transition) != len(b.transition) || len(a.output) != len(b.output) {
		return false
	}
	for i := 0; i < len(a.transition); i++ {
		if len(a.transition[i]) != len(b.transition[i]) {
			return false
		}
		for j := 0; j < len(a.transition[i]); j++ {
	...
	for i := 0; i < len(a.output); i++ {
		if len(a.output[i]) != len(b.output[i]) {
			return false
		}
		for j := 0; j < len(a.output[i]); j++ {
			if a.output[i][j] != b.output[i][j] {
				return false
			}
		}
	}
...
func printDOT(a automate) {
	fmt.Println("digraph {")
	fmt.Println("rankdir = LR")
	for i := 0; i < a.n; i++ {
		for j := 0; j < a.m; j++ {
			fmt.Printf("%d -> %d [label = \"%c(%s)\"]\n", i, a.transition[i][j], 'a'+j, a.output[i][j])
		}
	}
	fmt.Println("}")
}

func scanMealy(scanner *bufio.Scanner) automate {
	for scanner.Scan() {
		if scanner.Text() != "\n" || scanner.Text() != "" {
			break
		}
...
func main() {
	...
	automaton1 := scanMealy(scanner)
	automaton2 := scanMealy(scanner)
	automaton1.canonic()
	automaton2.canonic()
	...
	automaton1.canonic()
	automaton2.canonic()
	if automaton1.Equals(automaton2) {
		fmt.Printf("EQUAL")
	} else {
